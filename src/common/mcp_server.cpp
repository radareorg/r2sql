...
//
// See mcp_server.hpp for the threading model.
...
// Queue / timeout knobs (fixed constants).
static constexpr size_t kMcpDefaultMaxQueue = 64;
static constexpr int    kMcpDefaultQueueAdmissionTimeoutMs = 120000;
...
McpQueueResult McpServer::queue_and_wait(McpPendingCommand::Type type, const std::string& input) {
...

    {
        ...
        const size_t max_queue = kMcpDefaultMaxQueue;
        if (max_queue > 0 && pending_commands_.size() >= max_queue) {
            return {false, "Error: MCP queue is full"};
        }
...

    {
        ...
        const int timeout_ms = kMcpDefaultQueueAdmissionTimeoutMs;
        if (timeout_ms <= 0) {
            ...
            while (!cmd->completed && running_.load()) {
                ...
                return {false, "Error: MCP request timed out in queue"};
            }
        }
    }
...
int McpServer::start(int port, QueryCallback query_cb,
    ...
    Json query_input_schema = {
        ...
        {"properties", {
            {"query", {
                {"type", "string"},
                {"description", "SQL query or semicolon-separated script to execute against the radare2 session"}
            }}
        }},
    ...
    sql_query_tool.set_description("Execute a SQL query or semicolon-separated script against the radare2 session and return results");
    impl_->tool_manager.register_tool(sql_query_tool);

    std::unordered_map<std::string, std::string> descriptions = {
        {"r2sql_query", "Execute a SQL query or semicolon-separated script against the radare2 session and return results"}
    };
    ...
    // Mark the r2sql-side server state before exposing the SSE endpoint. The
...
void McpServer::run_until_stopped() {
    while (running_.load()) {
...
std::string format_mcp_info(int port, const std::string& bind_addr) {
    ...
    return "r2sql MCP server: http://" + rendered_host + ":" + std::to_string(port) + "/sse\n";
}
